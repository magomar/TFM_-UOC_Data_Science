...
General structure of compositional image captioning methods is depicted in Figure \ref{fig:compositional}. Usually, there are three components: 
\begin{enumerate}
...
\subsubsection{Describing novel objects}
...
% Very often, images posted in social media networks lack a proper description. In order to generate captions for these images, \citet{Park2017} propose a new method based on personalized views (who publishes an image) that is able to predict hashtags and generate image captions. This method uses a Context Sequence Memory Network (CSMN) to obtain the context information for a newly posted image. The proposed method exploits memory as a repository for multiple types of context information, captures long-term information without suffering from the vanishing gradient problem, and adopts CNN to jointly represent nearby ordered memory slots for better context understanding.

% Gaze indicates the cognition and perception of humans about a scene. Human gaze can identify the important locations of objects in an image. Thus, gaze mechanisms have already shown their potential performances in eye-based user modeling [18, 35, 109, 122, 124], object localization [100] or recognition [67] and holistic scene understanding [158, 160]. \citet{Sugano2016}claimed that gaze information has not yet been integrated in image captioning methods. This method introduced human gaze with the attention mechanism of deep neural networks in generating image captions. The method incorporates human gaze information into an attention-based LSTM model [152]. For experiments, it uses SALICON dataset [60] and achieves good results.
...
\subsection{Summary and benchmark results}
